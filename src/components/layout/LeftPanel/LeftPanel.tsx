...
import { ArrowLeftToLine, ChevronsUpDown, ArrowRightToLine, Github, Gitlab } from "lucide-react";
...
function LeftTop({
  ...
  return (
    <div className={`flex h-11 items-center px-5 transition-all duration-300 ${isCollapsed ? "justify-center px-0" : "justify-between w-full"}`}>
      {/* ปุ่มซ้าย */}
      ...
      {!isCollapsed && (
        <>
          ...
          <button onClick={onToggleAll} className="text-white/70 hover:text-white transition-colors shrink-0">
...
export default function LeftPanel({
  ...
  const toggleAllCategories = () => {
    const isAnyOpen = Object.values(categoriesOpen).some(isOpen => isOpen === true);
