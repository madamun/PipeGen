...
export async function GET(req: NextRequest) {
  ...
  const [meRes, reposRes] = await Promise.all([
    ...
    fetch("https://api.github.com/user/repos?per_page=100&sort=updated&visibility=all&affiliation=owner,collaborator,organization_member", {
      headers: ghHeaders,
      cache: "no-store",
    }),
  ]);
  ...
  if (!meRes.ok) {
    ...
    return Response.json({ error: `GitHub /user ${meRes.status}`, detail: t.slice(0, 200) }, { status: 502 });
  }
  if (!reposRes.ok) {
    const t = await reposRes.text();
    return Response.json({ error: `GitHub /user/repos ${reposRes.status}`, detail: t.slice(0, 200) }, { status: 502 });
  }
  ...
  const withMeta = await Promise.all(
    head.map(async (r) => {
      ...
      const [branchesRes, tagsRes, workflowsRes, langsRes] = await Promise.all([
        fetch(`https://api.github.com/repos/${r.full_name}/branches?per_page=1`, { headers: ghHeaders, cache: "no-store" }),
        fetch(`https://api.github.com/repos/${r.full_name}/tags?per_page=1`, { headers: ghHeaders, cache: "no-store" }),
        fetch(`https://api.github.com/repos/${r.full_name}/actions/workflows`, { headers: ghHeaders, cache: "no-store" }),
        fetch(`https://api.github.com/repos/${r.full_name}/languages`, { headers: ghHeaders, cache: "no-store" }),
      ]);
      ...
      return { ...r, _meta: { branchCount, tagCount, pipelineCount, languages } as const };
    })
  );
