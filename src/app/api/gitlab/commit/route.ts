...
import { prisma } from "@/server/db"; // หรือ @/server/prisma ตามโปรเจกต์คุณ
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (mode === "pull_request") {
            ...
            const createBranchRes = await fetch(`https://gitlab.com/api/v4/projects/${encodedId}/repository/branches`, {
                method: "POST",
                headers: {
        ...
        if (!commitRes.ok) {
            ...
            console.error("❌ Payload ที่ส่งไป:", JSON.stringify(commitPayload, null, 2)); // ปริ้นท์ของที่เราส่งไปดูด้วย
        ...
        if (mode === "pull_request") {
            const mrRes = await fetch(`https://gitlab.com/api/v4/projects/${encodedId}/merge_requests`, {
                method: "POST",
                headers: {
                    "Authorization": `Bearer ${account.accessToken}`,
                    "Content-Type": "application/json"
        ...
        return Response.json({ error: error.message || "Commit failed" }, { status: 500 });
    }
}