...
import { auth } from "@/server/auth"; // 👈 เช็ค path auth ให้ถูกนะครับ
...
export async function GET(req: Request) {
  try {
    ...
    if (!repoFullName || !filePath) return NextResponse.json({ content: "# Error: Missing parameters" });
    ...
    if (!repo) {
        ...
        return NextResponse.json({ content: "# Error: Repository not found in DB" });
    }
    ...
    if (!account?.accessToken) {
      ...
      return NextResponse.json({ content: `# Error: ${provider} Access Token not found (Check your login).` });
    }
    ...
    try {
        if (provider === "github") {
            ...
            const res = await fetch(apiUrl, { headers: { "Authorization": `Bearer ${account.accessToken}` } });
        ...
        return NextResponse.json({ content: `# Error fetching file: ${e.message}` });
    }
