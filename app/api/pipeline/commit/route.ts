  ...
  export async function POST(req: Request) {
    try {
      ...
      const { repoFullName, filePath, content, message, branch } =
        await req.json();
      ...
      if (!repo)
        return NextResponse.json(
          { error: "Repository not found in DB" },
          { status: 404 },
        );
      ...
      // ----------------------------------------------------
        ...
        if (!commitRes.ok) {
          ...
          return NextResponse.json(
            { error: "GitLab Error: " + (err.message || JSON.stringify(err)) },
            { status: 500 },
          );
        }
      }
